...
resource "google_compute_backend_service" "ml-glb-backends" {
  provider = "google.us-central1"
  name        = "ml-glb-backends"
  port_name   = "http"
  protocol    = "HTTP"
  timeout_sec = 30
  enable_cdn  = false
...
resource "google_compute_url_map" "ml-glb" {
  provider = "google.us-central1"
  name            = "ml-glb-url-map"
  default_service = "${google_compute_backend_service.ml-glb-backends.self_link}"
...
resource "google_compute_global_forwarding_rule" "ml-glb-ssl-forwarding-rule" {
  provider = "google.us-central1"
  name = "ml-glb-ssl-forwarding-rule"
